#!/usr/bin/env python3
"""
Trend Monitoring Agent (OpenAI version)
----------------------------------------
Runs on a schedule (see .github/workflows/trend-agent.yml). For every indicator
in roster.json, asks an OpenAI model (with the built-in web_search tool) to find
the latest value from the named source, classify it against the pre-defined
threshold bands, and return a structured result. Writes/updates data.json, which
the dashboard (index.html) reads on load.
...
MODEL = "gpt-5.5"  # swap for whichever current model your account has access to
